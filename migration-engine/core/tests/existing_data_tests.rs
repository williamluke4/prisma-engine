...
#[test_each_connector]
async fn adding_a_required_field_if_there_is_data(test_setup: &TestSetup, api: &dyn GenericApi) {
    let dm = r#"
            model Test {
...
#[test_each_connector]
async fn adding_a_required_field_must_use_the_default_value_for_migrations(test_setup: &TestSetup, api: &dyn GenericApi) {
        let dm = r#"
            model Test {
                id String @id @default(cuid())
            }
...
#[test_each_connector]
async fn dropping_a_table_with_rows_should_warn(test_setup: &TestSetup, api: &dyn GenericApi) {
        let dm = r#"
                    model Test {
                        id String @id @default(cuid())
                    }
                "#;
                let original_database_schema = infer_and_apply(test_setup, api, &dm).sql_schema;
...
#[test_each_connector]
async fn dropping_a_column_with_non_null_values_should_warn(test_setup: &TestSetup, api: &dyn GenericApi) {
        let dm = r#"
            model Test {
                id String @id @default(cuid())
                puppiesCount Int?
