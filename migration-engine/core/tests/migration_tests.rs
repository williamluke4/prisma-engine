...
#[test_each_connector]
async fn adding_a_many_to_many_relation_must_result_in_a_prisma_style_relation_table(
...
#[test_each_connector]
async fn adding_an_inline_relation_must_result_in_a_foreign_key_in_the_model_table(
    test_setup: &TestSetup,
    api: &dyn GenericApi,
) {
    let dm1 = r#"
...
#[test_each_connector]
async fn multi_column_unique_in_conjunction_with_custom_column_name_must_work(
    test_setup: &TestSetup,
    api: &dyn GenericApi,
) {
    let dm1 = r#"
...
#[test_each_connector(ignore = "Mysql")]
async fn updating_a_model_with_a_scalar_list_to_a_different_id_type_must_work(
    test_setup: &TestSetup,
    api: &dyn GenericApi,
) {
    let dm = r#"
...
#[test_each_connector]
async fn migrations_with_many_to_many_related_models_must_not_recreate_indexes(
    test_setup: &TestSetup,
    api: &dyn GenericApi,
) {
