...
#[test_each_connector]
async fn single_watch_migrations_must_work(test_setup: &TestSetup, api: &dyn GenericApi) {
    ...
    let migrations = migration_persistence.load_all().await;
...
#[test_each_connector]
async fn multiple_watch_migrations_must_work(test_setup: &TestSetup, api: &dyn GenericApi) {
    let migration_persistence = api.migration_persistence();
    ...
    let migrations = dbg!(migration_persistence.load_all().await);
...
#[test_each_connector]
async fn steps_equivalence_criteria_is_satisfied_when_leaving_watch_mode(test_setup: &TestSetup, api: &dyn GenericApi) {
        let migration_persistence = api.migration_persistence();
