...
#[test_each_connector]
async fn last_should_return_none_if_there_is_no_migration(test_setup: &TestSetup, api: &dyn GenericApi) {
...
#[test_each_connector]
async fn last_must_return_none_if_there_is_no_successful_migration(test_setup: &TestSetup, api: &dyn GenericApi) {
    let persistence = api.migration_persistence();
...
#[test_each_connector]
async fn load_all_should_return_empty_if_there_is_no_migration(test_setup: &TestSetup, api: &dyn GenericApi) {
    let persistence = api.migration_persistence();
...
#[test_each_connector]
async fn load_all_must_return_all_created_migrations(test_setup: &TestSetup, api: &dyn GenericApi) {
    let persistence = api.migration_persistence();
...
#[test_each_connector]
async fn create_should_allow_to_create_a_new_migration(test_setup: &TestSetup, api: &dyn GenericApi) {
...
#[test_each_connector]
async fn create_should_increment_revisions(_: &TestSetup, api: &dyn GenericApi) {
    let persistence = api.migration_persistence();
    let migration1 = persistence.create(Migration::new("migration_1".to_string())).wait();
    let migration2 = persistence.create(Migration::new("migration_2".to_string())).wait();
...
#[test_each_connector]
async fn update_must_work(test_setup: &TestSetup, api: &dyn GenericApi) {
    let persistence = api.migration_persistence();
