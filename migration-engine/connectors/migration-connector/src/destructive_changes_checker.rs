...
where
    T: Send + Sync + 'static,
{
    async fn check<'a>(&'a self, database_migration: &'a T) -> ConnectorResult<DestructiveChangeDiagnostics>;
}
...
where
    T: Send + Sync + 'static,
{
    async fn check<'a>(&'a self, _database_migration: &'a T) -> ConnectorResult<DestructiveChangeDiagnostics> {
